...
window.renderStatistics = function (ctx, names, times) {
  ...
  for (var i = 0; i < names.length; i++) {
    ...
    ctx.fillStyle = 'hsl(240, ' + number + '%, 50%)';

    if (names[i] === 'Вы') {
      ctx.fillStyle = 'rgba(255, 0, 0, 1)';
    }
    var startPointX = CLOUD_X + MARGIN + (BAR_WIDTH + MARGIN) * i;
    var startPointY = CLOUD_Y + FONT_GAP * 5 + (BAR_HEIGHT - (BAR_HEIGHT * times[i]) / maxTime);
    ctx.fillRect(startPointX, startPointY, BAR_WIDTH, (BAR_HEIGHT * times[i]) / maxTime);
